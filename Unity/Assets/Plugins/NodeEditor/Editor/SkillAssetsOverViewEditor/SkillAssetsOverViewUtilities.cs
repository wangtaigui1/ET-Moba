...
namespace AllTrickOverView.Core
{
    public static class SkillAssetsOverViewUtilities
    {
        ...
        {
            string[] targetsGuids = AssetDatabase.FindAssets("t:NPBehaveCanvas");
        ...
        private static void HandleRightClick(OdinMenuItem obj)
        {
            ...
            {
                NodeEditorWindow.OpenCanvasByPath(obj.Value as string);
            });
            genericMenu.AddItem(new GUIContent("在项目资源管理器高亮"), false, () =>
            {
                ...
                skillAssetsOverViewItem.NpBehaveCanvas = AssetDatabase.LoadAssetAtPath<NPBehaveCanvas>(obj.Value as string);
                EditorGUIUtility.PingObject(GetSkillAssetsOverViewItemByPath(obj.Value as string).NpBehaveCanvas);
            });
